...
// Now includes: Jump ring auto-deduction + confirmation, receipt sending,
// post-sale confirmation screen, chain_material_cost COGS tracking.
// Matches Event Mode feature parity (minus event-specific features like QR).
// ============================================================================
...
import type {
  InventoryItem, TaxProfile, PaymentMethod, ProductType, ChainProductPrice,
...
export default function StoreModePage() {
  ...
  // Pre-filter jump ring inventory
  ...
  // —— Add to cart (with jump ring metadata — PARITY with Event Mode) ——
  ...
  const addNonChainItem = (item: InventoryItem) => {
    cart.addItem({
      inventory_item_id: item.id, name: item.name, quantity: 1, unit_price: Number(item.sell_price),
  ...
  const addCustomItem = () => {
    ...
    cart.addItem({
      inventory_item_id: null, name: customItem.name, quantity: 1, unit_price: Number(customItem.price),
      discount_type: null, discount_value: 0, product_type_id: null, product_type_name: null, inches_used: null, pricing_mode: null,
  ...
  // —— Sale completion — with jump ring deduction + COGS ——
  ...
  const completeSale = async (resolutions: JumpRingResolution[]) => {
    ...
    try {
      ...
      const { data: sale, error: saleErr } = await supabase.from('sales').insert({
        tenant_id: tenant.id, event_id: null, client_id: clientId,
      ...
      for (const item of cart.items) {
        ...
        await supabase.from('inventory_items').update({ quantity_on_hand: Math.max(0, inv.quantity_on_hand - deduct) }).eq('id', item.inventory_item_id);
      }
      ...
      <header className="border-b border-[var(--border-default)] bg-white px-4 py-3 shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push('/dashboard')} className="p-2 rounded-lg hover:bg-[var(--surface-raised)] text-[var(--text-tertiary)] min-h-[44px] min-w-[44px] flex items-center justify-center"><BackArrow /></button>
            <div>
              <h1 className="text-base font-semibold text-[var(--text-primary)]">{tenant.name} — Store</h1>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 overflow-y-auto">
          <div className="p-4 md:p-6 max-w-2xl mx-auto">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      ...
                      {measureInches && Number(measureInches) > 0 && (
                        <div className="text-center">
                          <span className="text-2xl font-bold text-[var(--text-primary)] font-mono">${(Number(measureInches) * Number(selectedChain.sell_price)).toFixed(2)}</span>
            ...
            {step === 'tip' && (
              <div className="max-w-sm mx-auto py-8 space-y-6">
                ...
                <input type="number" step="0.01" min="0" className="w-full h-14 px-4 rounded-xl border border-[var(--border-default)] bg-white text-[var(--text-primary)] text-center text-xl font-mono focus:outline-none focus:border-[var(--border-strong)] focus:ring-[3px] focus:ring-[rgba(0,0,0,0.04)] transition-all"
                  placeholder="$0.00" value={cart.tip_amount || ''} onChange={(e) => cart.setTip(Number(e.target.value) || 0)} /></div>
              ...
              <div className="max-w-sm mx-auto py-8 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {PAYMENT_METHODS.map((pm) => (
                    <button key={pm.value} onClick={() => cart.setPaymentMethod(pm.value)} className={`py-7 rounded-2xl text-center transition-all min-h-[80px] ${cart.payment_method === pm.value ? 'bg-[var(--text-primary)] text-white shadow-md' : 'bg-white border border-[var(--border-default)] text-[var(--text-primary)] hover:border-[var(--border-strong)] hover:shadow-sm'}`}>
                      <div className="text-lg font-bold">{pm.label}</div>
                    </button>
                  ))}
                </div>
                {cart.payment_method && (
                  <button onClick={() => setStep('receipt')} className="w-full h-14 rounded-xl font-semibold text-base transition-all active:scale-[0.97] shadow-sm" style={{ backgroundColor: 'var(--accent-primary)', color: 'white' }}>Continue</button>
                )}
              </div>
            )}
            ...
            {step === 'receipt' && (
              <div className="max-w-sm mx-auto py-8 space-y-6">
                <h2 className={pageTitle + ' text-center'}>Receipt (Optional)</h2>
                <div className="space-y-4">
                  <div><label className="block text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--text-tertiary)] mb-2">Email</label>
                  <input className="w-full h-12 px-4 rounded-xl border border-[var(--border-default)] bg-white text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:border-[var(--border-strong)] focus:ring-[3px] focus:ring-[rgba(0,0,0,0.04)] transition-all"
                    type="email" value={receiptEmail} onChange={(e) => setReceiptEmail(e.target.value)} placeholder="customer@email.com" /></div>
                  <div><label className="block text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--text-tertiary)] mb-2">Phone (SMS)</label>
                  <input className="w-full h-12 px-4 rounded-xl border border-[var(--border-default)] bg-white text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:border-[var(--border-strong)] focus:ring-[3px] focus:ring-[rgba(0,0,0,0.04)] transition-all"
                    type="tel" value={receiptPhone} onChange={(e) => setReceiptPhone(e.target.value)} placeholder="+1 (555) 000-0000" /></div>
                </div>
                ...
                <button onClick={handleCompleteSale} disabled={processing}
                  className="w-full h-12 rounded-xl text-sm font-medium text-[var(--text-tertiary)] hover:text-[var(--text-secondary)] hover:bg-[var(--surface-subtle)] transition-colors">
                  Skip receipt and complete
                </button>
              </div>
            )}

            {/* ═══ CONFIRMATION STEP (NEW — matches Event Mode) ═══ */}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {receiptConfig.email && (
                        <div className="bg-white border border-[var(--border-default)] rounded-xl p-3 space-y-2">
                          ...
                          ) : (<><Button variant="secondary" size="sm" onClick={sendEmailReceipt} loading={sendingEmail} disabled={!receiptEmail} className="w-full min-h-[44px]">{sendingEmail ? 'Sending...' : 'Email Receipt'}</Button>{emailError && <p className="text-xs text-red-500">{emailError}</p>}</>)}
                        </div>
                      )}
                      {receiptConfig.sms && (
                        <div className="bg-white border border-[var(--border-default)] rounded-xl p-3 space-y-2">
                          ...
                          ) : (<><Button variant="secondary" size="sm" onClick={sendSMSReceipt} loading={sendingSMS} disabled={!receiptPhone} className="w-full min-h-[44px]">{sendingSMS ? 'Sending...' : 'Text Receipt'}</Button>{smsError && <p className="text-xs text-red-500">{smsError}</p>}</>)}
                        </div>
                      )}
                    </div>
                  </div>
