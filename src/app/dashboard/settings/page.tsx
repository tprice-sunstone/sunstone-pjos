...
function SettingsPage() {
  ...
  useEffect(() => {
    ...
    // Load supplier count for accordion summary
    fetch('/api/suppliers?tenantId=' + tenant.id)
      .then((r) => r.json())
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-6 space-y-3 pb-24">
      ...
      <AccordionSection
        ...
        title="Suppliers"
        summary={`${suppliers.length} supplier${suppliers.length !== 1 ? 's' : ''}`}
        isOpen={openSection === 'suppliers'}
        onToggle={() => toggleSection('suppliers')}
      >
        <div className="pt-4">
          <SuppliersSection tenantId={tenant.id} />
        </div>
      </AccordionSection>
