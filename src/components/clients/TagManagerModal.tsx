      ...
      <ModalBody>
        ...
        <div className="space-y-2">
          {tags.map((tag) => (
            <div key={tag.id}>
              {editingId === tag.id ? (
                <div className="flex items-center gap-2 p-2 bg-[var(--surface-subtle)] rounded-lg">
                  <input
                    value={editName}
                    onChange={(e) => setEditName(e.target.value)}
                    className="flex-1 text-sm px-2 py-1 border border-[var(--border-default)] rounded-md bg-[var(--surface-raised)] text-[var(--text-primary)]"
                ...
                <div className="flex items-center gap-3 px-3 py-2 hover:bg-[var(--surface-subtle)] rounded-lg group">
                  <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: tag.color }} />
                  <span className="flex-1 text-sm text-[var(--text-primary)]">{tag.name}</span>
                  <span className="text-xs text-[var(--text-tertiary)]">{tag.usage_count} clients</span>
                  <button
                    onClick={() => { setEditingId(tag.id); setEditName(tag.name); setEditColor(tag.color); }}
                    className="text-[var(--text-tertiary)] hover:text-[var(--text-primary)] opacity-0 group-hover:opacity-100 p-1"
                  >
                  ...
                  <button
                    ...
                    className="text-[var(--text-tertiary)] hover:text-red-500 opacity-0 group-hover:opacity-100 p-1"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
